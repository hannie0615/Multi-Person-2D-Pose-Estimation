...
class DataTransform():
    ...
    def __init__(self):
        ...
        self.data_transform = {
            'train': Compose([
                get_anno(),  # JSON에 어노테이션 저장
                add_neck(),  # 어노테이션 순서 변경
                aug_scale(),  # 확대/축소
                aug_rotate(),  # 회전
                aug_croppad(),  # 자르기
                aug_flip(),  # 좌우반전
                remove_illegal_joint(),  # 이미지에서 밀린 어노테이션 제거
                Normalize_Tensor()  # RGB 정보 정규화+텐서화
                # no_Normalize_Tensor()
            ]),
            'val': Compose([
                # validation 단계는 생략
...
class COCOkeypointsDataset(data.Dataset):
    """
    MSCOCO 데이터셋. pytorch dataset 상속
    ...
    def pull_item(self, index):
        ...
        # 6. 채널 순서 변경
        # 전：paf_mask：torch.Size([46, 46, 38])
        # 후 → torch.Size([38, 46, 46])
